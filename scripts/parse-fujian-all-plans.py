...
ROOT = Path("data/admission-plans/fujian-2026")
OUTPUT = ROOT / "fujian-2026-admission-plans.json"
...
def clean_major_name(value):
    ...
    kept = []
    for line in lines:
        if line.startswith(":") or "专业组" in line or line.startswith("http"):
            continue
        if len(line) == 1 and line in WATERMARK_CHARS:
            continue
        line = re.sub(r"^[福建省教育考试院]+", "", line).strip()
        if line:
            kept.append(line)
    return kept
...
def looks_like_school(value):
    text = clean(value)
    return bool(re.search(r"(大学|学院|职业技术大学|职业学院|高等专科学校|学校)", text))
...
def file_context(pdf_path):
    ...
    if "艺术" in name:
        ...
        track = "sports"
    elif "提前批" in name and "警察" in name:
        track = "police"
    else:
        track = "general"

    if "物理" in name or folder == "physics":
        subject_group = "物理科目组"
    elif "历史" in name or folder == "history":
        subject_group = "历史科目组"
    else:
        subject_group = ""

    batch = re.sub(r"^\d+", "", name)
    batch = batch.replace("——", " ")
    return track, subject_group, batch


def parse_pdf(pdf_path):
    records = []
    candidate_track, subject_group, batch = file_context(pdf_path)
    current_school = ""
    current_city = ""
    current_nature = ""
    current_group = ""
            ...
            for table in page.extract_tables() or []:
                for row in table:
                    ...
                    for index, code in enumerate(codes):
                        plan = int(plans[index]) if index < len(plans) else None
                        records.append(
                            {
                                "year": 2026,
                                "province": "福建",
...
all_records = []
for folder in ["physics", "history", "art-sports"]:
    for pdf in sorted((ROOT / folder).glob("*.pdf")):
        print("Parsing", pdf)
        all_records.extend(parse_pdf(pdf))

OUTPUT.write_text(json.dumps(all_records, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
print(f"Wrote {len(all_records)} records to {OUTPUT}")
print(f"High confidence: {sum(1 for item in all_records if item['confidence'] == 'high')}")
print(f"Needs review: {sum(1 for item in all_records if item['confidence'] != 'high')}")
